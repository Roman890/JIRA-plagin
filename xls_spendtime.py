...
def write_report(epics_list: list, tasks_list: list, subtasks_list: list, date_str: str):
    ...
    worksheet['A1'] = f'Отчет по трудозатратам команды за период ({date_str})'
    ...
    worksheet['K2'] = 'Затраченное время (дни)'
    ...
    for epic in epics_list:
        ...
        worksheet['E' + i] = epic.assignee
        worksheet['F' + i] = epic.status
        worksheet['G' + i] = epic.summary
        worksheet['H' + i] = epic.start_date
        worksheet['I' + i] = epic.due_date
        worksheet['J' + i] = epic.real_date
        worksheet['K' + i] = epic.spend_time
        column_num += 1
            ...
            if epic.key == task.epic.key:
                ...
                for subtask in subtasks_list:
                    ...
                    if task.key == subtask.task:
                        ...
                        worksheet['K' + i] = subtask.spend_time
                        column_num += 1
    excel_filename = date_str + '_report_spendtime.xlsx'
